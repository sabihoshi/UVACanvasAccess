...
namespace UVACanvasAccess.Util
{
    ...
    {
        [PublicAPI]
        public static bool IsHaltedState(this SisImportState state) => state == SisImportState.Aborted ||
...
#pragma warning disable 1998
        public static async IAsyncEnumerable<T> YieldAsync<T>(this T t) { yield return t; }
#pragma warning restore 1998
        ...
        public static string ToPrettyString<TK, TV>(this Dictionary<TK, TV> dictionary)
        {
            ...
            {
                var k = kIsPretty
                    ? ((IPrettyPrint) key).ToPrettyString()
                    : key.ToString();
                var v = vIsPretty
                    ? ((IPrettyPrint) val).ToPrettyString()
                    : val.ToString();
                sb.Append($"\n{k} -> {v},".Indent(4));
            }
        ...
        {
            var strings = IsA<IPrettyPrint, T>()
                ? enumerable.Cast<IPrettyPrint>().Select(e => e.ToPrettyString())
        ...
        {
            if (nullableT.HasValue) return nullableT.Value;
        ...
        /// <summary>
        ///     Pretty-prints this <see cref="IAsyncEnumerable{T}" /> using <see cref="CollectAsync{T}" /> and
        ///     <see cref="ToPrettyString{T}" />.
        /// </summary>
        /// <returns>The pretty string.</returns>
        [Pure]
        [PublicAPI]
        ...
        [Pure]
        internal static (TK, TO) ValSelect<TK, TV, TO>(this (TK, TV) kvp, Func<TV, TO> f) => (kvp.Item1, f(kvp.Item2));

        [Pure]
        internal static (TO, TV) KeySelect<TK, TV, TO>(this (TK, TV) kvp, Func<TK, TO> f) => (f(kvp.Item1), kvp.Item2);
        ...
        internal static IEnumerable<string> GetFlagsApiRepresentations([NotNull] this Enum en)
            => en.GetFlags().GetApiRepresentations();
        ...
        [Pure]
        internal static IEnumerable<T> Flatten<T>(this IEnumerable<IEnumerable<T>> ie) { return ie.SelectMany(e => e); }
        ...
        internal static IEnumerable<TE> GetFlags<TE>(this TE en) where TE : Enum
        {
            ...
            foreach (var value in Enum.GetValues(en.GetType()).Cast<TE>())
            {
                ...
                if (flag == bits && en.HasFlag(value)) yield return value;
            }
        }
        ...
        internal static IEnumerable<TO> SelectNotNull<TI, TO>([CanBeNull] [ItemCanBeNull] this IEnumerable<TI> ie,
            ...
            => ie?.DiscardNull().Select(f) ?? Enumerable.Empty<TO>();
        ...
        internal static IEnumerable<TO> SelectNotNullKey<TI, TO>([CanBeNull] [ItemCanBeNull] this IEnumerable<TI?> ie,
            ...
            => ie?.DiscardNullValue().Select(f) ?? Enumerable.Empty<TO>();
        ...
        internal static string GetApiRepresentation([NotNull] this Enum en)
        {
            ...
            return attribute.Length > 0
                ? ((ApiRepresentationAttribute) attribute[0]).Representation
                : null;
        }

        [Pure]
        internal static string GetString(this DiscussionTopicOrdering ordering) => ordering.GetApiRepresentation();
        ...
        [Pure]
        internal static string ToShortString(this bool b) => b
            ? "true"
            : "false";
        ...
        [ContractAnnotation("o:null => null; o:notnull => notnull")]
        [Pure]
        internal static TO? ConvertIfNotNullValue<TI, TO>([CanBeNull] this TI o, [NotNull] Func<TI, TO> f)
            where TO : struct => o == null
            ? (TO?) null
            : f(o);

        // C# lacks proper generic specialization which makes me sad. This is the best we have.
        [Pure]
        private static bool IsA<TInterface, TType>() => typeof(TInterface).IsAssignableFrom(typeof(TType));
    }
}